...
import com.comantis.bedBuzz.services.BillingSecurity.VerifiedPurchase;
import com.comantis.bedBuzz.services.C.ResponseCode;
...
public class BillingHelper {
	 ...
	 * @param itemId
	 */
	public static void requestPurchase(Activity activity, String itemId){
		if (amIDead()) {
			return;
		}
		...
		try {
			...
			startBuyPageActivity(pendingIntent, new Intent(), activity);
		} catch (RemoteException e) {
	...
	static void startBuyPageActivity(PendingIntent pendingIntent, Intent intent, Activity activity) {
	...
	protected static void verifyPurchase(String signedData, String signature) {
		...
		if (purchases.size() > 0)
		{
			...
			confirmTransaction(new String[]{latestPurchase.notificationId});
			 ...
			 Toast.makeText(mContext, "Thankyou for your purchase!",
	       	          Toast.LENGTH_SHORT).show();
		}
		else
		{
			 Toast.makeText(mContext, "Signing error",
	       	          Toast.LENGTH_SHORT).show();
		}
